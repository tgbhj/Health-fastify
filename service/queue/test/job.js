...
class Job extends EventEmitter {
    constructor(type, data, queue) {
        super();
        // console.log('[job.js.constructor() queue]', queue)
        /*
        * type:'email1'
        * data:{name:'zhangsan'}
        ...
        * */
        ...
        let object = await new Promise((resolve, reject) => {
            ...
            multi
                ...
                .hset(hkey, 'priority', this.priority) // 优先级(默认值0)
    ...
    * */
    addIntoJobs() {
        this.redis.client
            .zadd(this.redis.getKey('jobs', false), this.priority, this.getZid(), (err) => { // 有序集合q:jobs
                if (err) {
                    return super.emit('error', new Error(err))
                }
            })
    ...
    * */
    _toFailed(data) {
        ...
        multi
            ...
            .lpush(getKey('job', this.id, 'log')) // 添加日志q:job:id:log
            .exec((err) => {
                if (err) {
    ...
    _toRetry() {
        ...
        if (parseInt(this.max_attempts) > parseInt(this._attempts)) {
            ...
            multi
                ...
                .zadd(getKey('jobs', this.type, 'delayed'), this.priority, this.getZid()) // 添加q:jobs:email1:delayed
    ...
    _toActive(job) {
        ...
        return new Promise((resolve, reject) => {
            ...
            multi
                ...
                .zadd(getKey('jobs', 'active'), ttl || this.priority, zid) // 任务最大生命周期(一般使用这个，因为在xqueue.js.checkActiveTTL查询zrangebyscores) && 任务优先级
    ...
    _inactiveState(job) {
        ...
        return new Promise((resolve, reject) => {
            multi
                ...
                .zadd(getKey('jobs', this.type, this._state), this.priority, this.getZid())
                .zadd(getKey('jobs', this._state), this.priority, this.getZid())
                .exec((err) => {
                    if (err) {
                        return super.emit('error', new Error(err))
                    }
    ...
    * */
    _toDelay() {
        ...
        multi
            ...
            .zadd(getKey('jobs', this.type, 'delayed', false), this.priority, this.getZid()) // q:jobs:email1:delayed
            .exec((err) => {
                if (err) {
                    throw new Error(err)
                }
...
* 没找到在哪用到
* 4.在_toActive()中的multi.zadd(getKey('jobs', 'active'), ttl || this.priority, zid) // 为什么ttl || priority两个选择
*   ttl的作用是什么?
*   priority的作用是什么?
...
*             zadd('q:jobs:emails:inactive',this.priority,this.getZid())
*      结论: 后面的会覆盖前面的
*
* 7.加入workmap的作用是什么?
* */
...
*           添加q:jobs:active & (ttl || this.priority, zid)
...
*           zadd & q:jobs:active & (ttl || this.priority, zid)
